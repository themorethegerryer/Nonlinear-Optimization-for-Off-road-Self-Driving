...
function [u0, U] = MPC_iLQR(XDouble, XrefDouble, Uref)
    x0 = [XDouble(7) XDouble(8) XDouble(9) sqrt(XDouble(1)^2+XDouble(3)^2)
    Xref = ;

    model = KinematicBicycleModel();
    N = 100;
    dt = 0.1;
    Q = diag([10 10 100 1 0.1]);
    R = diag([1 1]);
    Qf = diag([100 100 1 1 0.1]);
